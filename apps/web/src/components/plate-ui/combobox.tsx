  ...
  ({ combobox, index, item, onRenderItem, className, ...rest }, ref) => {
    ...
    return (
      <div
        ref={ref}
        className={cn(
          'gap-2 relative flex h-9 cursor-pointer select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none transition-colors',
      ...
      >
        ...
        <EmployeeAvatar src={item.avatarUrl} name={item.value} size="small" />
        {props.children}
      </div>
    );
  },
